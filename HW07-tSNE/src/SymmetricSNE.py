...
def SymmetricSNE(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, max_iter=1000):
    """
        Runs symmetric SNE on the dataset in the NxD array X to reduce its
        dimensionality to no_dims dimensions. The syntaxis of the function is
        `Y = SymmetricSNE.SymmetricSNE(X, no_dims, perplexity), where X is an NxD NumPy array.
    """
...
if __name__ == "__main__":
    ...
    for perplexity in range(10, 200, 40): 
        print(perplexity)
        Y = SymmetricSNE(X, 2, 50, perplexity, max_iter=500)
